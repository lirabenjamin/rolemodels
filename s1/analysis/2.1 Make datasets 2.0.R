...
nominee_gpa = nominations %>% 
  ...
  group_by(nominator,name,time) %>% 
  nest() %>% 
  mutate(data = map(data,pull,nominee_gpa)) %>% 
...
nominator_gpa = nominations %>% 
  ...
  nest() %>% 
  mutate(data = map(data,pull,nominator_gpa)) %>% 
  mutate(m = map_dbl(data,mean,na.rm=T),
         sd = map_dbl(data,sd,na.rm = T),
         n = map_dbl(data,~sum(!is.na(.))))
...
colorder = c("StudyID","school","female","eth","ell","sped","frpl","time","age","scw_s","grit_s","grat_s","sci_s","aot_s","coregpa"     ,"avg_scw_tr"  ,"avg_grit_tr","avg_grat_tr","avg_sci_tr"  ,"avg_aot_tr"  ,"pn_aot","pn_bf","pn_grat","pn_grit","pn_sci","pn_scw")
