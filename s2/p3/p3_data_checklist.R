...
raw = qualtRics::fetch_survey("SV_5uKvunsROZQb7ka", force_request = TRUE)

data = raw %>% 
  # Drop people you don't want (previews, pilots, too young, attention checks...)
  filter(DistributionChannel == "anonymous") |> 
  filter(StartDate > lubridate::ymd_hms("2023-09-07 00:00:00")) |>
  filter(StartDate < lubridate::ymd_hms("2023-09-08 00:00:00")) %>%

  # Drop variables you wont need
  select(id = ResponseId,f_sc_self:suggestion_box)

original_n = data %>% nrow()

data = data %>% 
  select(-ends_with("NPS_GROUP"),-matches("_DO_")) %>%
  mutate_at(vars(f_sc_self:b_diff_friends), as.character) %>%
  pivot_longer(f_sc_self:b_diff_friends) %>% 
  filter(!is.na(value)) %>% 
  mutate(condition = case_when(
    str_starts(name, "f_") ~ "healthy eating",
    str_starts(name, "s_") ~ "saving",
    str_starts(name, "w_") ~ "work",
    str_starts(name, "b_") ~ "bedtime",
    T ~ "error"
  ),
  name = str_remove(name, "f_|s_|w_|b_"), 
  name = case_when(
...
data  <- 
  ...
  rename(
    ...
    mutate_at(vars(sc_self, standard10, standard5, standard0, friend1_minutes:diff_friends), as.numeric)
...
data = data %>%
  select(id, starts_with("bl"), suggestion_box, condition, sc_self, importance_self, standard10, standard5, standard0, friends_1:diff_friends) %>%
  pivot_longer(c(
  ...
  mutate(value = case_when( 
    ...
    condition == "work" ~ value,
    condition == "bedtime" ~ value*-1 + 180
  )) %>% 
  pivot_wider(names_from = name, values_from = value)
...
ggsave("s2/p3/figures/histograms.png", width = 10, height = 10)
...
write_rds(data.z, "s2/p3/data/lavaan_data.rds")
